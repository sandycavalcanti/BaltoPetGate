    ...
    await Promise.all([
      axios.get(urlAPI + 'selpessoa/' + id, { signal: controller.signal })
        .then(response => {
          ...
          setSelectPessoa({ "TB_PESSOA_NOME_PERFIL": error.response ? error.response.data.message : 'Houve um erro ao carregar o perfil' });
        }),
