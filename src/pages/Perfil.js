...
const Perfil = ({ navigation }) => {
  ...
  const HeaderHeight = useRef(perfilHeight);
  ...
  const SelecionarItens = async () => {
    await Promise.all([
      ...
      axios.post(urlAPI + 'selanimal/filtrar', { TB_PESSOA_ID: id }, { signal: controller.signal })
        .then(response => {
  ...
  useEffect(() => { scrollY.addListener(({ value }) => { const curRoute = routes[tabIndex].key; listOffset.current[curRoute] = value; }); headerScrollY.addListener(({ value }) => { listRefArr.current.forEach((item) => { if (item.key !== routes[tabIndex].key) return; if (value > HeaderHeight.current || value < 0) { headerScrollY.stopAnimation(); syncScrollOffset(); } if (item.value && value <= HeaderHeight.current) { item.value.scrollToOffset({ offset: value, animated: false }); } }); }); return () => { scrollY.removeAllListeners(); headerScrollY.removeAllListeners(); }; }, [routes, tabIndex]);
  const syncScrollOffset = () => { const curRouteKey = routes[_tabIndex.current].key; listRefArr.current.forEach((item) => { if (item.key !== curRouteKey) { if (scrollY._value < HeaderHeight.current && scrollY._value >= 0) { if (item.value) { item.value.scrollToOffset({ offset: scrollY._value, animated: false }); listOffset.current[item.key] = scrollY._value; } } else if (scrollY._value >= HeaderHeight.current) { if (listOffset.current[item.key] < HeaderHeight.current || listOffset.current[item.key] == null) { if (item.value) { item.value.scrollToOffset({ offset: HeaderHeight.current, animated: false }); listOffset.current[item.key] = HeaderHeight.current; } } } } }); };
  ...
  const renderHeader = () => {
    const y = scrollY.interpolate({ inputRange: [0, HeaderHeight.current], outputRange: [0, -HeaderHeight.current], extrapolate: 'clamp' });
    return (
      <Animated.View {...headerPanResponder.panHandlers} style={[styles.header, { transform: [{ translateY: y }], height: HeaderHeight.current, }]}>
  ...
  const renderTab1Item = ({ item, index }) => {
    return (
      <>
        <Perfil_post data={item} pessoal={pessoal.current} tipo="animal" />
  ...
  const renderTab2Item = ({ item, index }) => {
    return (
      <View style={{ backgroundColor: '#CEF7FF', justifyContent: 'space-around' }}>
        <Perfil_post data={item} pessoal={pessoal.current} tipo="post" />
        <Post data={item} />
      </View>
    );
  };
